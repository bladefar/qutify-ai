...
const styles = StyleSheet.create({
  ...
  label: { marginBottom: 5, fontSize: 8, color: "#71717A", textTransform: "uppercase" },
  ...
  footer: { position: "absolute", bottom: 30, left: 40, right: 40, color: "#A1A1AA", fontSize: 8 },
});
    ...
    <Document title={`Quotation ${quote.id.slice(0, 8)}`} author="Quotify AI">
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.customer}>{quote.customer_name ?? "No customer"}</Text>
        </View>
...
export function DownloadQuotationPdf({
  ...
  async function handleDownload() {
    ...
    try {
      ...
      link.href = url;
      link.download = `quotation-${quote.id.slice(0, 8)}.pdf`;
      link.click();
